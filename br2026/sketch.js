...
const createMap = (array, fontsize) => {
  ...
  for(var jj = 0; jj < array[0].length; jj++) {
    ...
    for(var ii = 0; ii < array.length; ii++) {
      ...
      } else if (['R', 'G', 'B', 'O'].indexOf(array[ii][jj]) != -1) {
        const pad = new Pad(array[ii][jj], fontsize)
        iMap[jj].push(pad)
        pads.push({x: jj, y: ii, pad: pad})
      } else if (['1', '2', '3', '4'].indexOf(array[ii][jj]) != -1) {
...
const createTouchControls = () => {
  ...
  function createControlButton(emoji, direction, container) {
  ...
  // Create the four direction buttons
  createControlButton('⬆️', 'U', buttonContainer)
  createControlButton('⬇️', 'D', buttonContainer)
  createControlButton('⬅️', 'L', buttonContainer)
  createControlButton('➡️', 'R', buttonContainer)
}
...
function setupSwipeControls() {
  ...
  canvasElement.addEventListener('touchend', (e) => {
    if (worldMap.gameComplete) return
...
const victoryMessage = () => {
    ...
    //msg.style('box-shadow', '0 0 20px rgba(0,0,0,0.3)')
    //msg.style('z-index', '1000')
    msg.style('text-align', 'center')
    //msg.style('font-family', 'Arial, sans-serif')
  return msg
}
...
function draw() {
  ...
  if (stage == 'starring') {
    ...
    fill('#FF1493')
    textSize(36)
    text('Ready to catch your rose? 🌹', wW / 2, wH / 2 - 30)
    fill('#888888')
    textSize(18)
    text('Press any key or button to start', wW / 2, wH / 2 + 30)
